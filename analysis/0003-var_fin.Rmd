...
```{r include=FALSE}
base::options(max.print = 3000)
base::unique(dataset$`DS_CONTA`)
```
...
```{r}
dataset <- dataset %>% dplyr::filter(CD_CONTA=="3.01"|CD_CONTA=="3.11")
```
...
```{r}
dataset_aj <- dataset %>% 
  dplyr::group_by(CD_CONTA, CD_CVM) %>% 
  dplyr::arrange(dataset, DT_REFER, .by_group = T) %>% 
...
```{r}
valor_conta <- base::as.numeric(dataset_aj$VL_CONTA)
```


Ajuste da amplitude dos valores em 'VL_CONTA':
```{r}
dataset_aj <- dataset_aj %>% dplyr::mutate(VALORES = valor_conta / 10000000000)
```


Exclusão de linhas duplicadas:
```{r}
dataset_aj <- dplyr::distinct(dataset_aj)
```


Validação de 4 contas por empresa (2 de receita e 2 de resultado):
```{r}
contagem <- dataset_aj %>% dplyr::count(CD_CVM, CD_CONTA)
```  
...
```{r}
dataset_aj <- dataset_aj %>% dplyr::filter(!(CD_CVM=="26034"&VALORES==451228))
```  
...
```{r}
cadastro <- readxl::read_excel("D:/datasets/cvm_data.xlsx")
```
...
```{r}
infinitos <- base::is.infinite(dataset_aj$VARIACAO)
nan <- base::is.nan(dataset_aj$VARIACAO)
